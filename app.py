...
UNSPLASH_API_KEY = 'STou1Txzm4W2VzWZAEWf2r8ZGzUxEE-_BOc8QQiGIjU'  # Replace if using Unsplash API directly
...
async def get_location_name_async(lat, lon, session):
    try:
        url = f'https://nominatim.openstreetmap.org/reverse?lat={lat}&lon={lon}&format=json'
        headers = {'User-Agent': 'SL-Voyager/1.0 (dpsdp04@gmail.com)'}  # Replace with your email
        ...
        logging.error(f"Error in reverse geocoding for {lat},{lon}: {e}")
        return 'Location name unavailable'
    
# Generate practical travel tips
def generate_travel_tips(weather, attraction_name):
    tips = []
    weather_desc = weather['description'].lower() if weather['description'] != 'Weather data unavailable' else ''
    temperature = weather['temperature']

    # Weather-based tips
    if 'rain' in weather_desc or 'shower' in weather_desc:
        tips.append("Warning: Heavy rain is expected. Consider postponing your visit or bringing waterproof gear (raincoat, umbrella, waterproof shoes).")
        tips.append("Check local weather updates before heading out, as conditions can change rapidly.")
    elif 'cloud' in weather_desc:
        tips.append("Expect cloudy skies. Bring a light jacket in case of cooler temperatures.")
    elif 'clear' in weather_desc or 'sun' in weather_desc:
        tips.append("Sunny weather expected. Wear sunscreen, sunglasses, and a hat to protect against UV rays.")
    else:
        tips.append("Weather conditions may vary. Check the latest forecast before your trip.")
# Temperature-based tips
    if temperature == 'N/A':
        tips.append("Weather data unavailable. Bring versatile clothing for potential changes in temperature.")
    elif temperature > 30:
        tips.append("Hot weather expected. Wear light, breathable clothing and stay hydrated.")
    elif temperature < 20:
        tips.append("Cooler temperatures expected. Bring a jacket or sweater.")

    # Cultural/religious tips for Sri Lanka
    if 'temple' in attraction_name.lower() or 'dagoba' in attraction_name.lower() or 'sacred' in attraction_name.lower():
        tips.append("This is a religious site. Remove hats and shoes before entering, and dress modestly (cover shoulders and knees).")
        tips.append("Maintain silence and respect during visits to avoid disturbing worshippers.")
    tips.append("Always carry a reusable water bottle and stay hydrated, especially during outdoor activities.")
    tips.append("Respect local customs: Avoid public displays of affection and ask for permission before photographing people.")

    return tips

async def get_unsplash_image_async(query, session):
    try:
        # Primary query: attraction-specific (e.g., "sri-lanka,sigiriya-rock")
        queries = [
            f"sri-lanka,{query}",  # Specific: e.g., "sri-lanka,sigiriya-rock"
            f"{query}",            # Attraction only: e.g., "sigiriya-rock"
            "sri-lanka"            # Fallback: generic Sri Lanka
        ]
        for q in queries:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status != 200:
                    logging.error(f"Unsplash API request failed with status {response.status} for query={q}")
                    continue  # Try next query
                data = await response.json()
                logging.debug(f"Unsplash response for query={q}: {data}")
                image_url = data.get('urls', {}).get('regular')
                if image_url:
                    return image_url
                logging.warning(f"No image URL in Unsplash response for query={q}: {data}")
        # Ultimate fallback
        logging.warning(f"No suitable image found for {query}, using default")
        return 'https://via.placeholder.com/800x400?text=Sri+Lanka+Attraction'
    except Exception as e:
        logging.error(f"Error fetching Unsplash image for query={query}: {str(e)}")
        return 'https://via.placeholder.com/800x400?text=Sri+Lanka+Attraction'

#New route for attraction details
@app.route('/attraction/<name>', methods=['GET'])
@login_required
async def attraction_detail(name):
    try:
        # Find attraction in dataset
        attraction = data[data['Name'] == name]
        if attraction.empty:
            logging.warning(f"Attraction not found: {name}")
            return render_template('attraction-detail.html', error="Attraction not found", active_page='attraction_detail')

        attraction = attraction.iloc[0]
        async with aiohttp.ClientSession() as session:
            # Fetch weather
            weather = await get_weather_async(attraction['Latitude'], attraction['Longitude'], session)
            # Fetch location name
            location_name = await get_location_name_async(attraction['Latitude'], attraction['Longitude'], session)
            # Fetch image
            image_url = await get_unsplash_image_async(name.lower().replace(" ", "-"), session)
            # Generate travel tips
            travel_tips = generate_travel_tips(weather, attraction['Name'])

        # Prepare data for template
        attraction_data = {
            'name': attraction['Name'],
            'description': attraction['Description'],
            'latitude': attraction['Longitude'],  # Swapped for Leaflet
            'longitude': attraction['Latitude'],  # Swapped for Leaflet
            'location_name': location_name,
            'weather': weather,
            'travel_tips': travel_tips,
            'image_url': image_url
        }
        return render_template('attraction-detail.html', attraction=attraction_data, active_page='attraction_detail')
    except Exception as e:
        logging.error(f"Error in attraction_detail: {str(e)}", exc_info=True)
        return render_template('attraction-detail.html', error=str(e), active_page='attraction_detail')
